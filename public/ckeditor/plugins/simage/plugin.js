CKEDITOR.plugins.add("simage", {
	icons: "simage",
	allowedContent:
		"img[alt,!src,width,height,data-width,data-height]{border-style,border-width,float,height,margin‌​,margin-bottom,margi‌​n-left,margin-right,‌​margin-top,width}",
			...
			exec: function(editor) {
				...
				a.onchange = function() {
					...
					if (file.size > 5000000) {
						...
						m.innerHTML =
							"Image size exceeded! Please upload image of less than 5 MB.";
						b.appendChild(m);
						...
						["jpeg", "jpg", "png", "svg", "gif", "tif", "svg+xml"].indexOf(
							file.type.split("/")[1]
						) === -1
					) {
						b = document.createElement("div");
						...
						m = document.createElement("span");
						m.innerHTML =
							"The uploaded image format is not of acceptible format! Please upload an image!";
						b.appendChild(m);
						c = document.createElement("span");
						c.className = "close";
						c.innerHTML = "X";
					...
					loaderHtmlStr =
						...
						'<div style="width: 100%;height: 30px;margin-top: 100px;">Please wait while image is uploading...</div>' +
						"</div>";
							...
							(function(_this) {
								return function(data, textStatus, jqXHR) {
									...
									if (jqXHR.status == 200) {
										...
										// url = editor.config.dataParser(file);
										url = img.src;
										var getUrl = window.location;
										var finalurl = getUrl.protocol + "//" + getUrl.hostname;
										let insertUrl =
											finalurl + "/marketing_crm/assets/upload/" + file.name;
										...
										newLine = CKEDITOR.dom.element.createFromHtml(
											"<p><br></p>"
										);
										console.log(editor.config.srcSet);
										if (editor.config.srcSet) {
											srcSet = editor.config.srcSet(data);
											imgElem =
												'<img src="' +
												url +
												'" class="image-editor" srcset="' +
												srcSet +
												'" data-width="' +
												inputWidth +
												'" data-height="' +
										...
										editor.insertElement(newLine);
										loaderElem.remove();
										$(CKEDITOR.instances[curr.name]).trigger(
											"enableFormSubmit"
										);
									}
								};
